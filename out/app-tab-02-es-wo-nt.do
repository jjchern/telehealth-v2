...
foreach out in dr_all_causes dr_cerebrovascular_diseases dr_diabetes_mellitus ///
		...
		eststo: qui areg ln_`out' `ctrls' i.year s3_es_year_l* s3_es_year_f* [aw = pop] if s3 == 1, ab(county) vce(cl state)
		* save es coef 
		preserve
		parmest, fast
		gen out = "`out'"
...
esttab using "out/app-tab-02-es-wo-nt.rtf", replace b(3) se(3) keep(s3_es_year_l* s3_es_year_f*) label star(** 0.05) varwidth(32) ///
	   s(pv bl_mean bl_mean_lvl n_units N, fmt(2 2 2 0 %9.0fc) l("{\i p}-values (Pre-Trend)" "Mean of DV (Period 0)" "Mean of DV (Period 0) in Level" "Number of States" "Number of Observations")) ti("1. WLS, s3")
...
l in 1/20
save "dta/app-tab-02-es-wo-nt-est.dta", replace
