...
foreach out in dr_all_causes dr_cerebrovascular_diseases dr_diabetes_mellitus ///
		...
		eststo: qui areg ln_`out' `ctrls' i.year c.year#i.state treat_post [aw = pop] if s3 == 1, ab(county) vce(cl state)	
...
esttab using "out/tab-05-st-linear.rtf", replace b(3) se(3) keep(treat_post) label star(** 0.05) varwidth(32) ///
	   s(bl_mean bl_mean_lvl n_units N, fmt(2 2 0 %9.0fc) l("Mean of DV (Pre-Event)" "Mean of DV (Pre-Event) in Level" "Number of States" "Number of Observations")) ti("1. WLS, s3")
...
foreach out in dr_all_causes dr_cerebrovascular_diseases dr_diabetes_mellitus ///
		...
		eststo: qui areg ln_`out' `ctrls' i.year##i.treat c.year#i.state treat_post [aw = pop] if s3 == 1 | c == 1, ab(county) vce(cl state)	
...
esttab, b(3) se(3) keep(treat_post) label star(** 0.05) varwidth(32) ///
	   s(bl_mean bl_mean_lvl n_units N, fmt(2 2 0 %9.0fc) l("Mean of DV (Pre-Event)" "Mean of DV (Pre-Event) in Level" "Number of States" "Number of Observations")) ti("1. WLS, s3")
esttab using "out/tab-05-st-linear.rtf", append b(3) se(3) keep(treat_post) label star(** 0.05) varwidth(32) ///
	   s(bl_mean bl_mean_lvl n_units N, fmt(2 2 0 %9.0fc) l("Mean of DV (Pre-Event)" "Mean of DV (Pre-Event) in Level" "Number of States" "Number of Observations")) ti("1. WLS, s3")
